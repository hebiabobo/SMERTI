...
# For squad dataset
def filter_squad_questions(path, max_length):
    question_list = []
        ...
        for data in jline['data']:
            ...
            for paragraph in data['paragraphs']:
                ...
                for qas in paragraph['qas']:
                    ...
                    if len(qas['question'].split()) <= max_length and qas['question'] != '' and isEnglish(
                            qas['question']) \
                            and 'http' not in qas['question'].lower() and 'www' not in qas['question'].lower():
                        clean_line = re.sub('\s+', ' ', qas['question']).strip()
                        clean_line_final = process_text(clean_line)
                        question_list.append(clean_line_final + '\n')
                    counter += 1
    return question_list


path = r"D:\PycharmProjects\SMERTI-master\train-v1.1.json"
max_length = 20
questions_list = filter_squad_questions(path, max_length)
...
nlp = StanfordCoreNLP(r'D:\PycharmProjects\stanford-corenlp-4.2.2')
...
def run_conversion(qas):
    ...
    for question, answers, context in qas:
        ...
        for rule in CONVERSION_RULES:
            ...
            if sent:
                if not OPTS.quiet:
                    print('  Rule "%s": %s' % (rule.name, colored(sent, 'green'))).encode('utf-8')
...
def generate_answers(qas):
    ...
    if not OPTS.quiet:
        print()
    print('=== Summary ===')
    print('Matched %d/%d = %.2f%% questions' % (num_matched, len(qas), 100.0 * num_matched / len(qas)))
    print()
    for rule_name, func in ANSWER_RULES:
        num = rule_counter[rule_name]
        print('  Rule "%s" used %d times = %.2f%%' % (rule_name, num, 100.0 * num / len(qas)))
    print()
