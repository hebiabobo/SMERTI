...
def bart_generation_mask_to_unmask(masked_sentence):
    if '<mask>' not in masked_sentence:
        ...
        #                                top_p=0.8)
        generated_ids = model.generate(input_ids, max_length=50, repetition_penalty=5.0)
...
def write_results(eval_lines, path_20, path_40, path_60, path_80, counter_start, dataset_name):
    ...
    for line in zip(eval_lines[0], eval_lines[1], eval_lines[2], eval_lines[3]):
        print("\nCurrently evaluating line {}".format(counter))
        print("Line: ", line)  # 这是list不是str

        # replacement_entity = line[0]
        masked_sentences = [[], [], [], []]
        masked_sentences[0] = line[0][0]
        masked_sentences[1] = line[1][0]
        masked_sentences[2] = line[2][0]
        masked_sentences[3] = line[3][0]
        ...
        mask_rate_20 = line[0][1]
        mask_rate_40 = line[1][1]
        mask_rate_60 = line[2][1]
        mask_rate_80 = line[3][1]

        if dataset_name == "News":
            # For news headlines dataset:
            # f_filter_input.write(line[0] + '\t' + line[1] + '\t' + RE + '\n')
            f_20.write(output_20 + '\t' + mask_rate_20 + '\n')
            f_40.write(output_40 + '\t' + mask_rate_40 + '\n')
            f_60.write(output_60 + '\t' + mask_rate_60 + '\n')
            f_80.write(output_80 + '\t' + mask_rate_80 + '\n')
...
def choose_dataset(Dataset_name):
    if Dataset_name == 'News':
        ...
        # path_mutimask = os.path.join(eval_dir, "ste-bart-mutimask-baseline")
        path_mask = os.path.join(eval_dir, "baseline-bart-large")
        masked_lines[0] = get_eval_lines(os.path.join(path_mask, "mask_sentences_20.txt"))
        masked_lines[1] = get_eval_lines(os.path.join(path_mask, "mask_sentences_40.txt"))
        masked_lines[2] = get_eval_lines(os.path.join(path_mask, "mask_sentences_60.txt"))
        masked_lines[3] = get_eval_lines(os.path.join(path_mask, "mask_sentences_80.txt"))
        # masked = str(masked_lines[0] + str('\t') + masked_lines[1] + '\t' + masked_lines[2] + '\t' + masked_lines[3] + '\n')
        # filtered_input_path = os.path.join(path_mutimask, "news_filtered_input.txt")
        path_20 = os.path.join(path_mask, "news_output_20.txt")
        path_40 = os.path.join(path_mask, "news_output_40.txt")
        path_60 = os.path.join(path_mask, "news_output_60.txt")
        path_80 = os.path.join(path_mask, "news_output_80.txt")
        ...
        path_20 = os.path.join(path_mask, "reviews_output_20.txt")
        path_40 = os.path.join(path_mask, "reviews_output_40.txt")
        path_60 = os.path.join(path_mask, "reviews_output_60.txt")
        path_80 = os.path.join(path_mask, "reviews_output_80.txt")
    ...
    write_results(masked_lines, path_20, path_40, path_60, path_80, 1, Dataset_name)
