    ...
    for hyper in hypers:
        ...
        for coordinate in coordinates:
            for l in coordinate.lemmas():  # 各种词形变换
                if lemmatizer.lemmatize(word) not in l.name().lower() and l.name().lower() not in lemmatizer.lemmatize(word):
                    print("l.name().lower()")
                    print(l.name().lower())
                    ...
                    if sim_score >= lemma_keyword_sim_max:
                        lemma_keyword_sim_max = sim_score  # 与当前被替换词具有最大语义相似度的coordinate的语义相似度分数
                        lemma_keyword_sim_max_name = l.name().lower()  # 与当前被替换词具有最大语义相似度的coordinate
...
def single_prompt_helper(keywords_lst, keywords_dict, fnc):
    ...
    for i in range(0, len(keywords_lst)):
        ...
        # if keyword_pos == 'j':
        #     keyword_pos = 'a'
        candidate, candidate_count = fnc(keyword, keyword_pos)  # 调用get_coordinates()获取每一个keyword的coordinate及语义相似度
        if candidate != "":
            if candidate_count >= candidate_max_count:
                candidate_max_count = candidate_count  # 再选语义相似度最高的keyword和coordinate的group
...
def choose_dataset(Dataset_name):
    if Dataset_name == 'News':
        ...
        path_20 = os.path.join(path_mutimask, "news_output_20.txt")
        path_40 = os.path.join(path_mutimask, "news_output_40.txt")
        path_60 = os.path.join(path_mutimask, "news_output_60.txt")
        path_80 = os.path.join(path_mutimask, "news_output_80.txt")
...
# OE_sentence = "my daughter enjoined the book . it was challenging , but with a little help she was able to read it all ."
# OE_sentence = "this book is excellent ! great stories and details about disneyland . a must have for any disney enthusiast !"
# OE_sentence = "this toy is very cute and cuddly ! it is the perfect size ! very durable and fun to play with !"
OE_sentence = "i really like this bag , the strap is very sturdy and it has lots of pockets and compartments for storage ."
# OE_sentence = "i love this shirt for my daughter ! it is very cute and the cotton was very comfy and soft ."
# OE, RE, RE_sentence = single_prompt_wordnet(OE_sentence)

OE = "bag"
RE = "pot"
RE_sentence = "i really like this pot , the strap is very sturdy and it has lots of pockets and compartments for storage ."
...
masked_sentences, mask_rates = main_USE_function(OE_sentence, RE_sentence, OE, RE)
print("masked_sentences")
print(masked_sentences)
