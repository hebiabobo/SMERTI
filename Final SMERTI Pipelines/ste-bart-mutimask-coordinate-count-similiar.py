...
def single_prompt_helper(keywords_lst, keywords_dict, fnc):
    ...
    for i in range(0, len(keywords_lst)):
        ...
        # if keyword_pos == 'j':
        #     keyword_pos = 'a'
        candidate = fnc(keyword, keyword_pos)  # 调用get_coordinates()获取每一个keyword的coordinate及语义相似度
        if candidate != "":
            try:
                for l in wordnet.synsets(candidate)[0].lemmas():
                    sim_score = get_similarity_score([l.name().lower(), keyword])
                    if sim_score >= lemma_keyword_sim_max:
                        lemma_keyword_sim_max = sim_score  # 再选语义相似度最高的keyword和coordinate的group
...
def get_masked_sentence(mask_threshold, similarity_threshold, length, similarity_vector_old, word_list,
    ...
    else:  # 如果当前masked的单词数大于句中所允许mask的最大单词数了   (不满足返回条件)
        counter = 0
...
def choose_dataset(Dataset_name):
    if Dataset_name == 'News':
        ...
        path_mutimask = os.path.join(eval_dir, "ste-bart-mutimask-coordinate-count-similiar")
